...
def Sawing(faza,case):
  pdf = canvas.Canvas('task.pdf', pagesize=A4)
  pdf.setTitle('')
...
def Calc(tabl3):
  ...
  tabl2 = []
  for i in tab:
    if (const - (90+12*i.count_tab)) >= 0:
      const = const - (90+12*i.count_tab)
      tabl2.append(i)
      tabl3.remove(i)
  return (tabl2,tabl3)
...
def Page(tabl,width,height):
  ...
  tabList = []
  for i in tabl:
    heighList.append(90+12*i.count_tab)
    height -= 90+12*i.count_tab
    tabList.append([Page1(i.task,width,90+12*i.count_tab)])
...
def Body(tabl,width,height):
  ...
  heighList.append(height)
  tab.append([''])
  table = Table(tab,colWidths=width/8,rowHeights=heighList)
  table.setStyle([
