from models import Drawing, Hole, Order, Point, Part, PointPart, Detail, AssemblyNorm, SawNorm, TaskPart, Weld, WeldNorm, HoleNorm, Task, TaskPart
...
def Task_create(faza,case):
  ...
  d = []
  tasks = PointPart.select(PointPart,Point,Part).join(Part).join(Drawing).join(Order).join_from(PointPart,Point).where(Order.id == case.id,Point.faza == faza).group_by(Part.profile,Part.size)
  for i in tasks:
    index += 1
    # task = Task(task = index,oper = i.part.work,profile = i.part.profile,size = i.part.size)
    task = Task(task = index,oper = i.part.work,faza = faza, order = case)
    task.save()
    taskparts = PointPart.select(PointPart,Part,fn.SUM(Part.count).alias('count_task')).join(Part).join(Drawing).join(Order).join_from(PointPart,Point).where(Order.id == case.id,Point.faza == faza,Part.profile == i.part.profile,Part.size == i.part.size).group_by(Part)
    for taskpart in taskparts:
      d.append((task,taskpart.part,taskpart.count_task))
  aaa = ((PointPart.hole,'hole'),(PointPart.bevel,'bevel'),(PointPart.notch,'notch'),(PointPart.chamfer,'chamfer'),(PointPart.milling,'milling'),(PointPart.bend,'bend'),(PointPart.turning,'turning'))
  for a in aaa:
