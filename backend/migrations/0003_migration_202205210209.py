...
def forward(old_orm, new_orm):
    ...
    reg = new_orm['reg']
    return [
        # Convert datatype of the field user.telegram: VARCHAR -> BIGINT,
        user.update({user.telegram: old_user.telegram.cast('SIGNED')}).where(old_user.telegram.is_null(False)),
        # Convert datatype of the field reg.telegram: VARCHAR -> BIGINT,
...
def backward(old_orm, new_orm):
    ...
    reg = new_orm['reg']
    return [
        # Convert datatype of the field user.telegram: BIGINT -> VARCHAR(100),
        user.update({user.telegram: old_user.telegram.cast('CHAR')}).where(old_user.telegram.is_null(False)),
        # Convert datatype of the field reg.telegram: BIGINT -> VARCHAR(100),
