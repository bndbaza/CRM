...
def forward(old_orm, new_orm):
    coating = new_orm['coating']
    return [
        # Apply default value 0 to the field coating.price,
        coating.update({coating.price: 0}).where(coating.price.is_null(True)),
    ]


def backward(old_orm, new_orm):
    pointpaint = new_orm['pointpaint']
    return [
        # Apply default value 0 to the field pointpaint.price,
        pointpaint.update({pointpaint.price: 0}).where(pointpaint.price.is_null(True)),
    ]
