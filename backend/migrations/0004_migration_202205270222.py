...
def forward(old_orm, new_orm):
    otc = new_orm['otc']
    return [
        # Apply default value False to the field otc.fix,
        otc.update({otc.fix: False}).where(otc.fix.is_null(True)),
        # Apply default value False to the field otc.usc,
        otc.update({otc.usc: False}).where(otc.usc.is_null(True)),
    ]
