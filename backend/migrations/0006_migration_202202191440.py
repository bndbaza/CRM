...
def forward(old_orm, new_orm):
    old_detail = old_orm['detail']
    detail = new_orm['detail']
    return [
        # Don't know how to do the conversion correctly, use the naive,
        detail.update({detail.norm: old_detail.norm}).where(old_detail.norm.is_null(False)),
    ]
...
def backward(old_orm, new_orm):
    ...
    detail = new_orm['detail']
    return [
        # Convert datatype of the field detail.norm: DECIMAL -> INT,
        detail.update({detail.norm: old_detail.norm.cast('SIGNED')}).where(old_detail.norm.is_null(False)),
    ]
