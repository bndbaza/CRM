...
def forward(old_orm, new_orm):
    ...
    reg = new_orm['reg']
    return [
        # Convert datatype of the field user.telegram: INT -> VARCHAR(100),
        user.update({user.telegram: old_user.telegram.cast('CHAR')}).where(old_user.telegram.is_null(False)),
        # Convert datatype of the field reg.telegram: INT -> VARCHAR(100),
        reg.update({reg.telegram: old_reg.telegram.cast('CHAR')}).where(old_reg.telegram.is_null(False)),
    ]
...
def backward(old_orm, new_orm):
    ...
    old_reg = old_orm['reg']
    return [
        # Apply default value 0 to the field user.telegram,
        user.update({user.telegram: 0}).where(user.telegram.is_null(True)),
        # Convert datatype of the field user.telegram: VARCHAR -> INT,
        user.update({user.telegram: old_user.telegram.cast('SIGNED')}).where(old_user.telegram.is_null(False)),
        # Apply default value 0 to the field reg.telegram,
        reg.update({reg.telegram: 0}).where(reg.telegram.is_null(True)),
        # Convert datatype of the field reg.telegram: VARCHAR -> INT,
        reg.update({reg.telegram: old_reg.telegram.cast('SIGNED')}).where(old_reg.telegram.is_null(False)),
    ]
