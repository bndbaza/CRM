...
class Drawing(peewee.Model):
    ...
    paint = CharField(default='1', max_length=40)
    class Meta:
        table_name = "drawings"
...
@snapshot.append
class CatalogSteel(peewee.Model):
    id = PrimaryKeyField(primary_key=True)
...
class Part(peewee.Model):
    ...
    area = DecimalField(auto_round=False, decimal_places=3, default=0, max_digits=12, rounding='ROUND_HALF_EVEN')
    class Meta:
        table_name = "parts"
...
class Detail(peewee.Model):
    ...
    faza = snapshot.ForeignKeyField(backref='details', index=True, model='faza')
    class Meta:
        table_name = "details"
...
class Shipment(peewee.Model):
    id = PrimaryKeyField(primary_key=True)
    order = snapshot.ForeignKeyField(backref='shipments', index=True, model='order', null=True)
    number = IntegerField()
...
class Hole(peewee.Model):
    ...
    count = IntegerField()
    depth = IntegerField()
    norm = snapshot.ForeignKeyField(backref='holes', index=True, model='holenorm', null=True)
    class Meta:
        table_name = "holes"


@snapshot.append
class Joint(peewee.Model):
    id = PrimaryKeyField(primary_key=True)
    paint = snapshot.ForeignKeyField(backref='joints', index=True, model='part')
    detail = snapshot.ForeignKeyField(backref='joints', index=True, model='detail')
    norm = DecimalField(auto_round=False, decimal_places=3, default=0, max_digits=12, rounding='ROUND_HALF_EVEN')
    class Meta:
        table_name = "joints"
...
@snapshot.append
class NeedForMetal(peewee.Model):
    id = PrimaryKeyField(primary_key=True)
    catalog_steel = snapshot.ForeignKeyField(backref='needformetals', index=True, model='catalogsteel')
    name_steel = CharField(max_length=100, null=True)
    receipt_date = DateTimeField(default=datetime.datetime.now)
    case = snapshot.ForeignKeyField(backref='needformetals', index=True, model='order')
...
@snapshot.append
class StoreSteel(peewee.Model):
    id = PrimaryKeyField(primary_key=True)
    catalog_steel = snapshot.ForeignKeyField(backref='storesteels', index=True, model='catalogsteel')
    width = IntegerField()
    length = IntegerField()
    name_steel = CharField(max_length=100)
    price = DecimalField(auto_round=False, decimal_places=3, default=0, max_digits=12, rounding='ROUND_HALF_EVEN')
    receipt_date = DateTimeField(default=datetime.datetime.now)
...
class Weld(peewee.Model):
    ...
    cathet = snapshot.ForeignKeyField(backref='welds', index=True, model='weldnorm')
    length = DecimalField(auto_round=False, decimal_places=3, max_digits=12, rounding='ROUND_HALF_EVEN')
    count = IntegerField()
    class Meta:
        table_name = "welds"
...
def forward(old_orm, new_orm):
    ...
    user = new_orm['user']
    return [
        # Convert datatype of the field user.ip: VARCHAR(16) -> VARCHAR(250),
        user.update({user.ip: fn.SUBSTRING(old_user.ip, 1, 250)}).where(old_user.ip.is_null(False)),
    ]
...
def backward(old_orm, new_orm):
    ...
    user = new_orm['user']
    return [
        # Convert datatype of the field user.ip: VARCHAR(250) -> VARCHAR(16),
        user.update({user.ip: fn.SUBSTRING(old_user.ip, 1, 16)}).where(old_user.ip.is_null(False)),
    ]
