<template>
  <div>
    <v-container fluid>
      <h1 v-if="($route.params.id).split(',')[0] != '0'" class="text-center">Заказ {{($route.params.id).split(',')[0]}} Марка {{($route.params.id).split(',')[1]}}</h1>
      <h1 v-else class="text-center">Марка {{($route.params.id).split(',')[1]}}</h1>
      <v-row>
